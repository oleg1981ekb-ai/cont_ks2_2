...
def get_status_text(num, total):
    ...
    return "[ОДОБРЕНО] Документы согласованы" if num == 1 else "[ОДОБРЕНО] Документы согласованы" if num == 1 else "[ОЖИДАНИЕ] Ждет старта" if total == 0 else ""
...
def apply_row_style(ws, row_idx, font, fill, border, alignment=None, is_center_cols=False):
    for idx in range(1, 8):
        ...
        elif is_center_cols and idx in (1, 4, 5, 6): cell.alignment = config.ALIGN_C
    ...
    for direction in mock_data.keys():
        ...
        for sub_obj in mock_data[direction].keys():
            ...
            for r_key in config.MONTHS_LIST:
                ...
                val = saved_sums[sum_key] if sum_key in saved_sums else mock_data[direction][sub_obj][r_key][0]
                for i, d_name in enumerate(config.DOCUMENTS_LIST):
                    ...
                    if hist_key in saved_statuses:
                        ws[f'D{current_row}'].value, ws[f'E{current_row}'].value, ws[f'F{current_row}'].value = saved_statuses[hist_key]
                    else:
                        def_status = mock_data[direction][sub_obj][r_key][1]
                        if def_status is not None: ws[f'D{current_row}'] = ws[f'E{current_row}'] = ws[f'F{current_row}'] = def_status
                    cur_f = ws[f'F{current_row}'].value
                    ws[f'G{current_row}'] = get_status_text(cur_f, val) if cur_f in (1,2,3) else ("[ОЖИДАНИЕ] Ждет старта" if val == 0 else "")
                    ...
                    for col_idx in (4, 5, 6): ws.cell(row=current_row, column=col_idx).alignment = config.ALIGN_C
                    current_row += 1
                end_doc = current_row - 1
                if ws[f'F{start_doc}'].value in (1,2,3): ws[f'H{mth_row}'] = ws[f'F{start_doc}'].value
                ws[f'C{mth_row}'] = f"=SUM(C{start_doc}:C{end_doc})"
                for col in ['D', 'E', 'F']: ws[f'{col}{mth_row}'] = f"=MAX({col}{start_doc}:{col}{end_doc})"
                ws[f'G{mth_row}'] = f'=IF(F{mth_row}=1; "[ОДОБРЕНО] Документы согласованы"; IF(F{mth_row}=2; "[В РАБОТЕ] На согласовании / проверке"; IF(F{mth_row}=3; "[ОЖИДАНИЕ] Еще не началось согласование"; "")))'
