            ...
            for r_key in config.MONTHS_LIST:
                ...
                for i, d_name in enumerate(config.DOCUMENTS_LIST):
                    ...
                    if hist_key in saved_statuses:
                        ...
                        ws[f'F{current_row}'].value = old_vals if isinstance(old_vals, (list, tuple)) and len(old_vals) > 0 else old_vals
                        ws[f'G{current_row}'].value = old_vals if isinstance(old_vals, (list, tuple)) and len(old_vals) > 0 else old_vals
