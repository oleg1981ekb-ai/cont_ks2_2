...
def save_config(data_rows, months, docs):
    """Автоматически пересобирает config.py с новыми заголовками СтрК, СДО, ГенДир"""
    with open("config.py", "w", encoding="utf-8") as f:
        ...
        # Измененные укороченные заголовки столбцов
        f.write('HEADERS = ["№ п/п", "Наименование объекта / Месяц / Документ", "Сумма (руб.)", "СтрК", "СДО", "ГенДир", "Текущий статус акта"]\n')
...
def run_wizard():
    while True:
        ...
        if choice == "1":
            ...
            # Принудительно перезаписываем конфиг один раз для применения новых заголовков при первом пуше
            ...
            subprocess.run(["git", "commit", "-m", "Columns layout optimization"])
