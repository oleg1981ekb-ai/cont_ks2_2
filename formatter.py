...
def apply_conditional_formatting(ws):
    ...
    # Диапазон условного форматирования расширен до колонки G (теперь D, E, F, G)
    ws.conditional_formatting.add("D2:G1000", CellIsRule(operator='equal', formula=['1'], fill=f_grn, font=font_grn))
    ws.conditional_formatting.add("D2:G1000", CellIsRule(operator='equal', formula=['2'], fill=f_ylw, font=font_ylw))
    ws.conditional_formatting.add("D2:G1000", CellIsRule(operator='equal', formula=['3'], fill=f_red, font=font_red))
...
def set_column_widths(ws):
    ...
    # Добавлен четвертый статус-столбец G (ГенДир Один экс.) с минимальной шириной 7
    ws.column_dimensions['D'].width = ws.column_dimensions['E'].width = ws.column_dimensions['F'].width = ws.column_dimensions['G'].width = 7
    # Текстовый статус сместился на колонку H
